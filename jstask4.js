...
//gameArrayオブジェクト
//quizzes 取得した問題のデータを入れている配列達
//index　今何問目？
//correctAnswerCount正解数カウント
class QuizData {
  //コンストラクタ　配列の初期化
  constructor(quizData) {
    gameArray.quizzes = quizData.results;
    gameArray.index = 0;
    gameArray.correctAnswerCount = 0;
  }
  //クイズがいま何問目かを判断し10問目以下なら出題ファンクション、10問目以上だったらリザルト画面を表示するゾッド
  countQuiz() {
    if (gameArray.quizzes.length > gameArray.index) {
      questionElement.hidden = false;
      questionElement.textContent = gameArray.quizzes[gameArray.index].question;
      this.setQuiz();
    } else {
      this.finishQuiz();
    }
  ...
  setQuiz() {
    ...
    headLine.textContent = "問題" + (gameArray.index + 1);
    questionElement.textContent = gameArray.quizzes[gameArray.index].question;
    genreElement.textContent = "[ジャンル]" + gameArray.quizzes[gameArray.index].category;
    difficultyElement.textContent = "[難易度]" + gameArray.quizzes[gameArray.index].difficulty;
    this.makeAnswer(gameArray.quizzes);
  }
